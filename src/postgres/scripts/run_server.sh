...
elif [[ "${AUTH_MODE}" == "shared-token" || "${AUTH_MODE}" == "none" ]]; then
...
if [[ "${AUTH_MODE}" == "request-credential" ]]; then
  echo "==> 已启用 request-credential 模式"
  echo "==> 客户端需在每次请求中携带 ${RequestSecretIDHeader:-X-TencentCloud-Secret-Id} / ${RequestSecretKeyHeader:-X-TencentCloud-Secret-Key}"
