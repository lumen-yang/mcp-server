...
> **当前主分支默认模式已切换为 `request-credential`**：客户端每次请求通过 Header 直传 `SecretId/SecretKey`。若你要继续使用旧的 `issued-token` / Bearer token 版本，请改看 `variants/token-oauth-authorization-version` 副本内文档。

本文面向希望**自行部署并接入 MCP 客户端**的用户，覆盖本地运行、Docker 部署、当前默认的请求头凭据模式，以及兼容旧模式时的注意事项。
...
- **MCP 客户端**：支持 `streamable-http` 类型 MCP Server 的客户端
...
| `MCP_SERVER_HTTP_ENDPOINT` | 否 | streamable-http 路径，默认 `/mcp` |
| `MCP_STREAMABLE_HTTP_STATELESS` | 否 | 是否启用无状态 streamable-http，默认建议 `true` |
| `MCP_SERVER_PUBLIC_URL` | 否 | 打印给客户端使用的完整 MCP URL |
...
```bash
MCP_ACCESS_TOKEN=<MCP_ACCESS_TOKEN> go run ./cmd/mcp_smoke --url http://127.0.0.1:9000/mcp --region ap-guangzhou
```

### 2. 验证真实只读云 API 调用

```bash
MCP_ACCESS_TOKEN=<MCP_ACCESS_TOKEN> go run ./cmd/verify --url http://127.0.0.1:9000/mcp --region ap-guangzhou --instance-id postgres-xxxxxxxx
```
