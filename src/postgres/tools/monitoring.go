...
func RegisterMonitoringTools(s *server.MCPServer, cred *common.Credential, g *security.Guard) {
	// DescribeSlowQueryList - 查询慢查询列表（只读）
	registerTool(s, cred, g, "DescribeSlowQueryList", "查询慢查询列表",
		security.LevelNone,
		[]mcp.ToolOption{
	...
	registerTool(s, cred, g, "DescribeSlowQueryAnalysis", "慢查询分析",
		security.LevelNone,
		[]mcp.ToolOption{
			mcp.WithString("DBInstanceId", mcp.Required(), mcp.Description("实例ID")),
			mcp.WithString("StartTime", mcp.Description("开始时间")),
	...
	registerTool(s, cred, g, "DescribeDBErrlogs", "查询错误日志",
		security.LevelNone,
		[]mcp.ToolOption{
			mcp.WithString("DBInstanceId", mcp.Required(), mcp.Description("实例ID")),
			mcp.WithString("StartTime", mcp.Description("开始时间")),
