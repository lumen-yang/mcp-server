...
func RegisterDatabaseTools(s *server.MCPServer, cred *common.Credential, g *security.Guard) {
	// ===== 现有迁移（2个）=====

	// DescribeDatabases - 查询数据库列表（只读）
	registerTool(s, cred, g, "DescribeDatabases", "查询实例的数据库列表",
		security.LevelNone,
		[]mcp.ToolOption{
	...
	registerTool(s, cred, g, "CreateDatabase", "创建数据库",
	...
	registerTool(s, cred, g, "ModifyDatabaseOwner", "修改数据库属主",
		security.LevelAudit,
		[]mcp.ToolOption{
	...
	registerTool(s, cred, g, "DescribeDatabaseObjects", "查询数据库对象列表",
		security.LevelNone,
		[]mcp.ToolOption{
			mcp.WithString("DBInstanceId", mcp.Required(), mcp.Description("实例ID")),
